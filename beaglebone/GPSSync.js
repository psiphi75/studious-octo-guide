...
GPSSync.prototype.capture = function capture() {
    const self = this;
    // let gpsQualityType; // Stores the GPS Quality type.
    // let numGPSevents = 0;
    ...
    function setEventHandler(gpsNum) {
        self.gps[gpsNum].gps.on('position', position => {
    ...
    function wrapUp() {
        self.position = self.gps['1'].tmpPosition;
        console.log('GPSS: wrapUp() 1:', self.position);
        // console.log(
        //     `GPSS: wrapUp() 2: "${gpsQualityType}"`,
        //     JSON.stringify([self.gps['1'].tmpPosition, self.gps['2'].tmpPosition])
        // );
